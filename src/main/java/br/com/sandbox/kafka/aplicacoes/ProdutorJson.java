    ...
    public void executar() {
        ...
        try (KafkaProducer<String, String> producer = new KafkaProducer<>(props)) {
            ...
            for (long i = 1; i <= totalMensagens; i++) {
                try {
                    // Gerar mensagem JSON de ~2MB (mesma estrutura do Avro)
                    String mensagemJson = GeradorMensagemJson.gerarMensagem2MB(i);

                    String chave = "msg-" + i;
                    ProducerRecord<String, String> record = 
                        new ProducerRecord<>(TOPICO_MENSAGENS, chave, mensagemJson);
